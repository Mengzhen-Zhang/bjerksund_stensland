...
pub fn bs_call_iv(
    ...
    let f = |x| {
        let model_price = bs_call_price(spot, strike, x, risk_free_rate, dividend_rate, time_to_maturity);
        price - model_price
    };
    let mut convergency = roots::SimpleConvergency { eps:1e-5f64, max_iter:200 };
    let mut root = roots::find_root_brent(1e-5f64, 1e-2f64, &f, &mut convergency);
    if root.is_err() {
        root = roots::find_root_brent(1e-2f64, 1e-1f64, &f, &mut convergency);
    }
    if root.is_err() {
        root = roots::find_root_brent(1e-1f64, 1f64, &f, &mut convergency);
    }
    if root.is_err() {
        root = roots::find_root_brent(1f64, 10f64, &f, &mut convergency);
    }
    root
}
...
pub fn bs_put_iv(
    ...
    let f = |x| {
        let model_price = bs_put_price(spot, strike, x, risk_free_rate, dividend_rate, time_to_maturity);
        price - model_price
    };
    let mut convergency = roots::SimpleConvergency { eps:1e-5f64, max_iter:200 };
    let mut root = roots::find_root_brent(1e-5f64, 1e-2f64, &f, &mut convergency);
...
mod tests {
    ...
    fn test_bs_call_price() {
        ...
        let q = 0.03;
        let t = 1.0;
        let model_price = bs_call_price(s, k, v, r, q, t);
        let market_price = 13.5;
        let iv = bs_call_iv(s, k, r, q, t, market_price).unwrap();
        assert_eq!(model_price, 1.0_f64, "Price is {}\nIV is {}", model_price, iv);
    }
}
